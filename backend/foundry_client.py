...
class FoundryClient:
    """Client for Azure AI Foundry Agent with Fabric tool integration"""
    ...
    def _get_agent_token(self) -> str:
        """Retrieve agent access token from Key Vault"""
        if not self.kv_client:
            # Fallback to environment variable if Key Vault not configured
    ...
    def chat(self, user_id: str, message: str) -> Dict[str, Any]:
        """
        Send chat message to Azure AI Foundry Agent.
        The agent uses Fabric tool to query Fabric Data Agent.
        """
        if not self.project_endpoint or not self.agent_id:
            raise ValueError(
                "Azure AI Foundry configuration missing. "
                "Set AZURE_AI_FOUNDRY_PROJECT_ENDPOINT and AZURE_AI_FOUNDRY_AGENT_ID"
            )

        token = self._get_agent_token()
        if not token:
            raise ValueError("Agent token not found in Key Vault or environment")
        ...
        # Adjust based on actual Foundry API response format
        answer = ""
